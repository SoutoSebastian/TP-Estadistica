...
Para el test imperfecto consideramos un modelo donde la variable observada  \(T\) proviene de un test con sensibilidad \(Se\) y especificidad \(Sp\).
...
Además, la teoría indica que, como el estimador es insesgado, su varianza es:
...
Para  $\theta = 0.25$, $S_{e} = 0.9$ y $S_{p} = 0.95$, construyo muestras bootstrap para observar la distribución del estimador de momentos de $\theta$ cuando $n = 10$. 
...
* ¿Qué pasa si se achica la muestra? A continuación se realizan las mismas simulaciones variando n.
